...
1. **The engine is much smaller than the repository.** 71 `.sas` files containing 133
   ...
   unreachable files** — the whole `sas/etl/` layer, all 12 portfolio override books, all
   20 regulatory report programs and the orphaned TTC scorecard — are never invoked by any
   driver. The 113th never-called definition, `%months_between`, sits in the reached
   `m_util_dates.sas`. See [04](04_never_invoked.md).
